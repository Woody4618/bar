    ...
    <div className='bg-white shadow-md rounded-2xl border-solid border border-black w-[430px] text-center flex flex-col mx-auto p-4'>
      ...
      <div className='flex flex-col gap-2'>
        {receipts && receipts.receipts.map((receipt: any) => (
          <div key={receipt.receiptId} className='flex flex-col items-center p-2 border-b last:border-b-0'>
            <div className='font-medium'>Receipt #{receipt.receiptId.toString()}</div>
            <div className={receipt.wasDelivered ? 'text-green-600' : 'text-yellow-600'}>
            ...
            <div className='text-sm font-medium text-blue-600'>
              Price: {receipt.price.toString()} SOL
            </div>
          </div>
        ))}
      </div>
